...
- **Addresses** point to accounts on the Solana network. Anyone can read the
  data in a given address. Most addresses are also **public keys**.
...
In the same way that finance apps typically do math in cents (for USD), pence
...
connection with a specific Solana network, called a 'cluster'.

For now we'll use the `Devnet` cluster rather than `Mainnet`. `Devnet` is
designed for developer use and testing, and `DevNet` tokens don't have real
value.
...
The balance returned is in _lamports_, as discusssed earlier. Web3.js provides
...
Make a new file called `check-balance.ts`, substituting your public key in for
`<your public key>`.
...
Save this to a file, and `npx esrun check-balance.ts`. You should something
like:
